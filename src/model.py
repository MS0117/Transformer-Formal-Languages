...
from src.components.cape_transformer import CAPETransformerModel
...
class LanguageModel(nn.Module):
    ...
    def _initialize_model(self):
        ...
        elif self.config.model_type == 'CAPE-SAN':
            self.model = CAPETransformerModel(self.voc.nwords, self.voc.noutputs,
                                        self.config.d_model, self.config.heads,
                                        self.config.d_ffn, self.config.depth,
                                        self.config.dropout, pos_encode = self.config.pos_encode,
                                        bias = self.config.bias, pos_encode_type= self.config.pos_encode_type,
    ...
    def trainer(self, source, targets, lengths, hidden, config, device=None ,logger=None):
        ...
        elif config.model_type == 'CAPE-SAN':
        ...
        if config.model_type != 'SAN' and config.model_type != 'SAN-Simple' and config.model_type != 'SAN-Rel' and config.model_type != 'CAPE-SAN':
    ...
    try:
        for epoch in range(1, config.epochs + 1):
            ...
            for batch, i in enumerate(range(0, len(train_loader), config.batch_size)):
                if config.viz and (config.model_type == 'SAN' or config.model_type == 'CAPE-SAN') and batch % viz_every == 0:
