...
class CAPE1d(nn.Module):
    def __init__(self, device,d_model: int, max_global_shift: float = 0.0, max_local_shift: float = 0.0,
        ...
        cos_shifts = _sin2cos_phase_shift * (torch.arange(d_model) % 2).to(self.device)
    ...
    def forward(self, x: Tensor, x_lengths: Optional[Tensor] = None,
        ...
        return (x.to(self.device) * self.content_scale.to(self.device)) + self.compute_pos_emb(x, x_lengths, positions_delta).to(self.device)
    ...
    def compute_pos_emb(self, x: Tensor, x_lengths: Optional[Tensor] = None,
        ...
        positions = repeat(torch.arange(n_tokens),
                           't -> new_axis t', new_axis=batch_size).to(x).to(self.device)
        ...
        if positions_delta is None:
            ...
            positions *= positions_delta.to(self.device)
        ...
        product = positions * self.freq.to(x).to(self.device)

        pos_emb = torch.sin(product + self.cos_shifts.to(x).to(self.device))
    ...
    def set_content_scale(self, content_scale: float):
        self.content_scale = Tensor([content_scale]).to(self.device)
